...
namespace GameFW.Nav
{
    public class NavAOI
    {
        ...
        public NavAOI(float left, float bottom, float width, float height, float tileSize) {
            this.left = left;
            this.bottom = bottom;
            this.width = MathTool.CeilToInt(width / tileSize);
            this.height = MathTool.CeilToInt(height / tileSize);
        ...
        public List<int> GetPolyListInAOI(float x, float z) {
            int i = MathTool.FloorToInt((x - left) / tileSize);
            int j = MathTool.FloorToInt((z - bottom) / tileSize);
